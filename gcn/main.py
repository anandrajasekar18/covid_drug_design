...
def trainModels(data_type):
    ...
    for k in range(num_folds):
        if data_type == 'folds':
            ...
            train_df = pd.read_csv('Data/splits/AID1706_binarized_sars_scaffold/train.csv')
            test_df = pd.read_csv('Data/splits/AID1706_binarized_sars_scaffold/test.csv')
            val_df  = pd.read_csv('Data/splits/AID1706_binarized_sars_scaffold/dev.csv')
            print('Creating train molecule graphs')
            X_train = makeData(train_df['smiles'])
            print('Creating test molecule graphs')
            X_test  = makeData(test_df['smiles'])
            print('Creating val molecule graphs')
            X_val   = makeData(val_df['smiles'])
            y_train = np.array(train_df['activity'])
            y_test  = np.array(test_df['activity'])
            y_val   = np.array(val_df['activity'])
        ...
        if X_val != None:
            val_loader = mydataloader.makeDataLoader(X_val,y_val,True,batch_size) # Convert to PyTorch Dataloader

        model = Model(n_features,device,net_type)
        for epoch in (range(1, num_epochs)):
            train_loss = model.train(train_loader)
            if X_val != None:
                val_loss = model.test(val_loader)
                y_pred, y_true, acc, y_pred_prob = testModel(model,val_loader)
                roc_auc,prc_auc,_,_ = getAUC(y_pred_prob,y_true)
                print(f'Epoch:{epoch} \t Train Loss:{train_loss} \t Val Accuracy:{val_loss} \t ROC:{roc_auc} \t PRC:{prc_auc}')
            torch.save(model.net.state_dict(),base_path+extra_path+'test_'+str(epoch)+'.pt')
...
extra_path = args.extra_path + 'pretrain_' + str(data_type)+ '_' + str(net_type) + '/'
...
tl,tst_l = trainModels(data_type)
if data_type != 'SARS':
    ...
    print('ROC:\t',roc_auc_arr)
    print('PRC:\t',prc_auc_arr)
    print('ACC:\t',acc_arr)
