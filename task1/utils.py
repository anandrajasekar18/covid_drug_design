...
class Meter(object):
    ...
    def roc_auc_score(self):
        ...
        for task in range(n_tasks):
            ...
            task_y_pred = y_pred[:, task][task_w != 0].numpy()
            #NOTE Check multi_class type
            scores.append(roc_auc_score(task_y_true, task_y_pred,multi_class='ovr'))
        return scores
...
class EarlyStopping(object):
    ...
    def __init__(self, mode='higher', patience=10, filename=None):
        if filename is None:
            ...
            filename = 'Trained_models/early_stop_{}_{:02d}-{:02d}-{:02d}.pth'.format(
...
def load_dataset_for_classification(args):
    ...
    if args['dataset'] == 'Ecoli':
        ...
        dataset = Ecoli(args['path'],smiles_to_bigraph, args['atom_featurizer'])
    ...
    if args['dataset'] == 'Ecoli_MIT' or args['dataset'] == 'pseud':
        ...
        val_set   = Ecoli(args['val_path'],smiles_to_bigraph, args['atom_featurizer'])
        dataset   = train_set
...
def load_model(args):
    if args['model'] == 'GCN':
        #NOTE add similar imports for all other classfiers
        model = dgl_classfiers.GCNClassifier(in_feats=args['in_feats'],
